...
DiskSpaceWarning=安裝程式需要至少 %1 KB 的磁碟空間，您所選取的磁碟只有 %2 KB 可用空間。%n%n您仍要繼續安裝嗎？
...
UninstalledMost=%1 解除安裝完成。%n%n某些元件無法移除，您可以自行移除這些元件。
...
ConfirmDeleteSharedFile2=系統顯示下列共用檔案已不再被任何程式所使用，您要移除此共用檔案嗎？%n%n倘若您移除了以上檔案但仍有程式需要使用它，將造成這些程式無法正常執行，因此您若無法確定請選擇 「否」。保留此檔案在您的系統中不會造成任何損害。
